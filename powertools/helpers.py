...
@app.template_filter()
def maybe_wrap(s, pre, post=None):
    if s:
        return pre + s + (pre if post is None else post)
    else:
        return s
...
def tvdb_url(search_info):
    return escape("https://www.thetvdb.com/series/{}".format(search_info["slug"]))
...
def get_next_url(nxt=no_arg_sentinel):
    if nxt is no_arg_sentinel:
...
_true_vals = frozenset({"true", "t", "yes", "y", "on", "1"})
_false_vals = frozenset({"false", "f", "no", "n", "off", "0"})


def parse_bool(s, default=no_arg_sentinel):
    s = s.lower()
    if s in _true_vals:
        return True
    elif s in _false_vals:
        return False
    else:
        if default is no_arg_sentinel:
            raise ValueError(f"can't parse bool value {s!r}")
        else:
            return default
...
def login(browser, retry=True):
    ...
    if form is None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".html", delete=False) as f:
            ...
            raise ValueError(
                "no login form (HTTP {}); response in {}".format(
...
def open_with_login(browser, url):
    ...
    if error_div:
        ...
        if "is not available" in msg:
            raise ValueError(
                "I'm logged in but don't have permissions "
                + "({})\n{}".format(url, msg)
            )
        elif not browser.response.ok:
            raise ValueError(
                "Got {} for '{}':\n{}".format(browser.response.status_code, url, msg)
            )
    ...
    try:
        ...
        if e.args and e.args[0] in {"No state", "Wrong site"}:
            ...
            if not browser.response.ok:
                raise RoboError("Bad response: {}".format(browser.response.status_code))
        else:
            raise
